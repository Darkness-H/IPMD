# Libreria waitress para lanzar la aplicacion como un servidor en vez de un servidor de desarrollo
...
def trafico(airport):
    # Valores disponibles para airport (mayusculas o minusculas)
    airports = ['A-1', 'AP-1', 'A-8', 'AP-68', 'A-15']

    # Funcion que devuelve el ultimo parte de incidencias de trafico en la autopista seleccionada
    def make_request(airport):

        # Pedimos informacion a traves de una API
        response = requests.get('https://api.euskadi.eus/traffic/v1.0/incidences?_page=1')
        data = response.json()

	# Obtener todas las incidencias de la pagina 1
        # Las incidencias estan ordenadas de manera que los objetos de menor indice son los mas recientes
        incidences = data['incidences']

	# Guardaremos la incidencia en la variable obj
        obj = None

	# Iterar sobre las incidencias y extraer la ultima informacion de la autopista seleccionada
        for incidence in incidences:

	    # Salimos del bucle si encontramos la instancia mas reciente
    ...
    # Funcion que imprime los mensajes correspondientes
    def get_info(airport, airports=airports):
	    ...
	    # Si en la primera pagina no se encuentra ninguna instancia de la autopista seleccionada
            if (obj == None):

                return "There are no recent incidences about the airtport: {}".format(airport.upper())
	    ...
	    # En el caso de que el aeropuerto introducido no es valido
    	    return "Please enter one of the following airports"
    ...
    if (isinstance(obj,str)):
        ...
        html = "<h3>Trafico</h3>" \
		"<b>Mensaje:</b> Datos de la ultima parte de incidencias de trafico en la autopista {autopista}<br/>" \
		"<b>Datos:</b><br/>"
	...
	# Iterate over the key-value pairs in the obj
        for key, value in obj.items():
            ...
            html += "<b>{key}:</b> {value}<br/>".format(key=key, value=value)

        return html.format(autopista=airport.upper())

@app.route("/tiempo/<city>")
def tiempo(city):
    # Valores disponibles para city (mayusculas o minusculas)
    cities = ["Bilbao", "Donostia", "Vitoria"]

    # Funcion que devuelve la ultima prediccion de temperaturas maximas y minimas para la ciudad seleccionada
    def make_request_weather(city, cities=cities, API_KEY=API_KEY):

        # Parametros de consulta con la API key
        ...
        # Obtenemos la ultima prediccion de temperaturas
    ...
    def get_info_weather(city, cities=cities, API_KEY=API_KEY):
        ...
        if (city_aux in cities):
            ...
            # En el caso de que la ciudad introducida no es valida
            return  None, None
    ...
    if (max == None and min == None):
        ...
        html = "<h3>Tiempo</h3>" \
                "<b>Mensaje:</b> Prevision de temperaturas en {city}<br/>" \
                "<b>Maxima:</b> {max}<br/>" \
                "<b>Minima:</b> {min}"
...
if __name__ == "__main__":
    ...
    serve(app, host='0.0.0.0', port=696)
